<template>
    <div class="px-0 tracking-tight md:px-4 lg:px-6">
        <div class="py-4 bg-white border-b border-gray-200 mdx1:flex sm:justify-between lg:px-8">
            <h1 class="text-xl font-semibold text-gray-800">Order's List</h1>

            <form @submit.prevent="filteringSupport" class="flex-wrap items-end gap-3 mt-2 sm:flex">
            ...
            <div class="text-xs card">
                ...
                <div v-else class="min-h-80">
                    ...
                    <DataTable v-else ref="dt" :value="products" v-model:selection="selectedUsers" dataKey="id"
                        ...
                        <Column field="user" header="Name of user" sortable class="min-w-[10rem] whitespace-nowrap">
                        </Column>
                        <Column field="tel" header="Phone Number" sortable class="min-w-[10rem] whitespace-nowrap">
                        </Column>
                        <Column field="article" header="Article" sortable class="min-w-[10rem] whitespace-nowrap">
                        </Column>
                        <Column header="Status">
                            <template #body="slotProps">
                                <Tag v-if="slotProps.data.status" value="Handled" severity="success" />
                                <Tag v-else value="Pending" severity="warning" class="cursor-pointer"
                                    @click.prevent="toggleOverlayPanel" />

                                <OverlayPanel ref="op" class="bg-white cursor-pointer hover:bg-emerald-300">
                                    <p @click.prevent="(e) => handlingStatusToggle(e, slotProps.data.id)">
                                        Handled
                                    </p>
                                </OverlayPanel>
                            </template>
...
const filteringSupport = () => {
    ...
    axiosInstance
        .post(route("settings.order.list"), {
